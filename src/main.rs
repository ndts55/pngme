...
fn main() -> io::Result<()> {
    let arguments: Args = Args::from_args();
    match arguments {
        Args::Encode {
            file_path,
            chunk_type,
        ...
        Args::Print { file_path } => print(file_path),
    }
}
...
fn decode(file_path: PathBuf, chunk_type: String) -> io::Result<()> {
...
fn remove(file_path: PathBuf, chunk_type: String) -> io::Result<()> {
    let mut png = read_file(file_path.clone())?;
...
fn print(file_path: PathBuf) -> io::Result<()> {
    let png = read_file(file_path)?;
    println!("{}", png);
    Ok(())
}
